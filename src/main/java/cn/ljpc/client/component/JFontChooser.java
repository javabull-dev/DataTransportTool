...
public class JFontChooser extends JComponent {
	...
	// JFontChooser.class.getSimpleName() 获取类名
	private ResourceBundle messageCatalog = ResourceBundle.getBundle(JFontChooser.class.getSimpleName() + "Messages",
			getLocale());
	...
	protected String _(String key) {
		...
		try {
			try {
				//properties文件默认以ISO-8859-1编码，但是我编写的properties文件是以UTF-8格式编码的
				// messageCatalog.getString(key)已ISO-8859-1进行解码，getBytes("ISO-8859-1")对解码的字符又进行编码
				//new String(byte[], charset)对字节数组以UTF-8进行正确的解码
				value = new String(messageCatalog.getString(key).getBytes("ISO-8859-1"), Charset.forName("utf8"));
	...
	protected JDialog createDialog(Component parent) {
		...
		dialog.setSize(screenSize.width/2, screenSize.height/3*2);
